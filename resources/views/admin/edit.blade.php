<x-admin-layout>
    <h1 class="text-3xl font-bold">Blog Edit</h1>

    <form enctype="multipart/form-data" action="/admin/blogs/{{$blog->id}}/update" method="POST"  class="my-3">
        @csrf
        @method('PUT')
        <img src="/storage{{$blog->photo}}" alt="" class="w-80 h-60">
        <div>
        ...
        <div>
            ...
            <div class="relative mt-2 rounded-md shadow-sm">
                ...
                <input value="{{old('title', $blog->title)}}" type="text" name="title" id="title" class="block w-full rounded-md border-0 py-1.5 pl-7 pr-20 text-gray-900 ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6">
            </div>
            @error('title')
                <p class="text-red-500 text-xs">{{$message}}</p>
            @enderror
        ...
        <div>
            ...
            <div class="relative mt-2 rounded-md shadow-sm">
                ...
                <input value="{{old('slug' , $blog->slug)}}" type="text" name="slug" id="slug" class="block w-full rounded-md border-0 py-1.5 pl-7 pr-20 text-gray-900 ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6">
            </div>
            @error('slug')
                <p class="text-red-500 text-xs">{{$message}}</p>
            @enderror
        ...
        <div>
            ...
            <textarea  class="shadow-md border border-2 p-3 w-full my-3 resize-none" id="body" cols="20" rows="10" name="body">{{old('body' , $blog->body)}}</textarea>
            @error('body')
                <p class="text-red-500 text-xs">{{$message}}</p>
            @enderror
        </div>
